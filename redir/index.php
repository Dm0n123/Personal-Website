<?php
    ...
    $json = json_decode(file_get_contents("http://freegeoip.net/json/" . $_SERVER['REMOTE_ADDR']));
    $country = $json->country_code;
    $type = str_replace('"', "", $_GET['type']);

    $links = [
        "GPU" => "https://www.amazon.ca/gp/product/B01KMVHB6M/ref=as_li_tl?ie=UTF8&camp=15121&creative=330641&creativeASIN=B01KMVHB6M&linkCode=as2&tag=devoncrawford-20&linkId=e224ff8d8df8b445b63aa5bc0ec2f7d9",
    ...
    $direct = convertLocalLink($type, $country, $links);

    function convertLocalLink($type, $country, $links) {
        $rt = $links[$type];

        if(strcmp($country, "US") == 0) {
            if(strcmp($type, "2415") == 0) {
                $rt = "https://www.amazon.com/Canon-24-105mm-USM-Zoom-Lens/dp/B000B84KAW/ref=sr_1_3?ie=UTF8&qid=1518289159&sr=8-3&tag=devoncrawfo05-20";
            }
            else {
                $rt = str_replace("www.amazon.ca", "www.amazon.com", $rt);
                $rt = str_replace("tag=devoncrawford-20", "tag=devoncrawfo05-20", $rt);
            }
        }
        return $rt;
    }
